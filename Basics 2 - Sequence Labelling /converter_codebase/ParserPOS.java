...
public class ParserPOS {
    ...
    protected String tokenTagSeparate(String tokenTag){
      ...
      if(slash < 0)
        ...
        return tokenTag.substring(0,slash) + " "+ tokenTag.substring(slash + 1,tokenTag.length());
    }
      	...
      	for (int i = 0; i < files.length; i++) {
      	    ...
      	    if (!file.isDirectory()) {
          		  if (!file.getName().contains("CHANGES.LOG"))
          		      sentences.addAll(new ParserPOS(file).tokenLists());
      	    }
      	    else
      	    {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(files[0].isDirectory()){
          ...
          for(File file :files){
            if(!file.getName().contains("CHANGES.LOG")){
              ...
              if(!sentences.isEmpty()){
                ...
                FileWriter writer = new FileWriter(new File("wsj_readable/wsj_original",writeFile));
          ...
          // Output the sentences in a file
          //File dir = new File("atis_mallet");
          //dir.mkdir();
          FileWriter writer = new FileWriter(new File("atis_readable/atis_original","atis3.pos"));
