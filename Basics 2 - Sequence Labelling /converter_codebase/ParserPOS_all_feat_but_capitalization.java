...
public class ParserPOS_all_feat_but_capitalization {
    ...
    private static List <String>prefix = new ArrayList<String>();
    /** Create an object for a given LDC POS tagged file */
    public ParserPOS_all_feat_but_capitalization(File file) {
      	this.file = file;
      	try {
    ...
    public static void populatePrefix(String filename){
      try{
        int slash = filename.lastIndexOf("/");
        String directory = filename.substring(0,slash);
        filename = filename.substring(slash + 1, filename.length());
        ...
        s.close();
          ...
          System.out.println("Could not load prefix");
      }
      ...
      if(slash < 0)
        ...
        */
        ...
        for(int i = 0; i < prefix_length(); i++){
          if(word.startsWith(prefix.get(i))){
            pref = " " + prefix.get(i);
            break;
          }
        }
        ...
        return word+pref+suff+hyphen+number+" "+tag_word;
      }
      	...
      	for (int i = 0; i < files.length; i++) {
      	    ...
      	    if (!file.isDirectory()) {
          		  if (!file.getName().contains("CHANGES.LOG"))
          		      sentences.addAll(new ParserPOS_all_feat_but_capitalization(file).tokenLists());
      	    }
      	    else
      	    {
    ...
    public static void main(String[] args) throws IOException {
      // Update the prefix and suffix list
      populatePrefix(args[0]);
      populateSuffix(args[1]);
      ...
      if(files[0].isDirectory()){
        ...
        for(File file :files){
          if(!file.getName().contains("CHANGES.LOG")){
            ...
            if(!sentences.isEmpty()){
              ...
              FileWriter writer = new FileWriter(new File("wsj_readable/wsj_all_feat_but_capitalization",writeFile));
        ...
        //dir.mkdir();
        FileWriter writer = new FileWriter(new File("atis_readable/atis_all_feat_but_capitalization","atis3.pos"));
