...
public class ParserPOS_all_features {
    ...
    public ParserPOS_all_features(File file) {
      	this.file = file;
      	try {
    ...
    protected String tokenTagSeparate(String tokenTag){
      ...
      if(slash < 0)
        ...
        */
        ...
        return word+pref+suff+capital+hyphen+number+" "+tag_word;
      }
      	...
      	for (int i = 0; i < files.length; i++) {
      	    ...
      	    if (!file.isDirectory()) {
          		  if (!file.getName().contains("CHANGES.LOG"))
          		      sentences.addAll(new ParserPOS_all_features(file).tokenLists());
      	    }
      	    else
      	    {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if(files[0].isDirectory()){
          ...
          for(File file :files){
            if(!file.getName().contains("CHANGES.LOG")){
              ...
              if(!sentences.isEmpty()){
                ...
                FileWriter writer = new FileWriter(new File("wsj_readable/wsj_all_features",writeFile));
          ...
          //dir.mkdir();
          FileWriter writer = new FileWriter(new File("atis_readable/atis_all_features","atis3.pos"));
